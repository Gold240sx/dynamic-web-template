...
export function CompanyReviews() {
  ...
  const { data: reviews } = api.companyReview.getAll.useQuery(undefined, {
    refetchInterval: 30000, // Refetch every 30 seconds
  });

  const { mutate: createReview } = api.companyReview.create.useMutation({
    onSuccess: () => {
      setContent("");
      setRating(5);
      startTransition(() => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    createReview({ content: content.trim(), rating });
  };
